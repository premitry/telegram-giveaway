...
export async function createGiveaway(db: D1Database, data: Required<Omit<WizardData, 'image_file_id' | 'publish_chat_id' | 'description' | '_edit' | '_msgs'>> & WizardData): Promise<number> {
  const res = await db
    .prepare(
